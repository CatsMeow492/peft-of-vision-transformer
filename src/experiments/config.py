"""
Configuration system for PEFT experiments with YAML support and validation.
"""
...
try:
    ...
    from ..training.peft_trainer import TrainingConfig
...
class ConfigValidator:
    ...
    @staticmethod
    def check_compatibility(config1: ExperimentConfig, config2: ExperimentConfig) -> Tuple[bool, List[str]]:
        """
        Check if two configurations are compatible for comparison.
        ...
        if config1.model.name != config2.model.name:
            reasons.append("Different models")
        
        # Check if datasets are comparable
        if config1.dataset.name != config2.dataset.name:
            reasons.append("Different datasets")
        
        # Check if training settings are comparable
        if config1.training.num_epochs != config2.training.num_epochs:
            reasons.append("Different training epochs")
